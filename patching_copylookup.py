...
def build_copy_seq(logits: torch.Tensor, inputs: dict, texts:list[list[str]]) -> list[list[str]]:
    batch_size = logits.size(0)
    predictions = torch.argmax(logits, dim=-1).detach()
    box_preds, box_token_counts = compute_box_predictions(predictions.to(logits.device), inputs["token_in_bboxes"], logits.size(1))
    box_preds[box_token_counts < 4] = 0
    keep = box_preds == 1
    keep_indexes = keep.nonzero()
    filtered_texts = [[] for _ in range(batch_size)]
    for b, i in keep_indexes.tolist():
        if i > 0 and keep[b, i-1]:
            filtered_texts[b][-1] += ' '+texts[i][b]
        else:
            filtered_texts[b].append(texts[i][b])
    return filtered_texts


class CopyLookupCandidateGeneratior(CandidateGenerator):
    # TODO: load the layout analysis model and make the copy seq here?
    def __init__(
        self,
        copy_seqs: List[LongTensor],
        num_output_tokens: int = 10,
        max_matching_ngram_size: int = None,
        max_length: int = 20,
    ) -> None:
        self.copy_seqs = copy_seqs
        self.num_output_tokens = num_output_tokens
        self.max_matching_ngram_size = max_matching_ngram_size if max_matching_ngram_size else 2
        self.max_length = max_length
        self.seq = torch.cat(self.copy_seqs).view(1,-1)
        self.index = torch.cumsum(torch.tensor([s.size(0) for s in self.copy_seqs]), dim=0).to(self.seq.device) - 1
    ...
    def get_candidates(self, input_ids: LongTensor) -> Tuple[torch.LongTensor, Optional[torch.FloatTensor]]:
        ...
        for ngram_size in range(min(self.max_matching_ngram_size, input_length - 1), 1, -1):
            ...
            windows = self.seq.unfold(dimension=1, size=ngram_size, step=1)
            ...
            for idx in match_indices:
                ...
                if start_idx < self.seq.size(1):
                    i = torch.searchsorted(self.index, idx)
                    end_idx = start_idx + self.num_output_tokens
                    end_idx = min(end_idx, self.index[i]+1, input_length, self.max_length)
                    if start_idx < end_idx:
                        chosen_ids = self.seq[0, start_idx:end_idx]
                        match_found = True
                        self.on_top = i
                        break
            if match_found:
    ...
    def update_candidate_strategy(self, input_ids: LongTensor, scores: FloatTensor, num_matches: int):
        if num_matches > 0:
            #print(num_matches)
            self.copy_seqs = self.copy_seqs[self.on_top:] + self.copy_seqs[:self.on_top] # round the decoding copy sequence to top
            self.seq = torch.cat(self.copy_seqs).view(1,-1)
            self.index = torch.cumsum(torch.tensor([s.size(0) for s in self.copy_seqs]), dim=0).to(self.seq.device) - 1
...
def _get_candidate_generator(
    ...
    """
    if "copy_seqs" in model_kwargs:
        candidate_generator = CopyLookupCandidateGeneratior(
            copy_seqs= [seq.to(input_ids.device) for seq in model_kwargs["copy_seqs"]],
