...
from token_path_prediction.dataset.feature_processor.base_processor import CodeDocBaseProcessor
from token_path_prediction.modules.doc_utils import *
from token_path_prediction.modules.utils import distance
from torchvision.datasets.folder import pil_loader



class FilterPointerProcessor(CodeDocBaseProcessor):
    '''
    ...
    def __init__(self,
        ...
        self.dummy_filter_label_id = 0
        self.id2label = ['[DUMMY]', 'K', 'D', 'I']
        self.label2id = {k:v for v,k in enumerate(self.id2label)}
        # K: keep all
        # D: delete all, and set content attention to 0
        # IB: insert before
        # IA: insert after
    ...
    def process_label(self, sample, seg_se_map):
        ...
        # TPP2NER label #
        _grid_labels = np.zeros((1, self.max_text_length, self.max_text_length), dtype=int)
    ...
    def process_text(self, sample, return_ref=False, return_tensor=True):
        ...
        self.img_path = json_obj['img']['path']
        ...
        (input_ids,
         ...
         global_1d_positions,
         local_1d_positions,
         labels) = [self.tokenizer.cls_token_id], [1], [1], [self.dummy_bbox], [
            self.dummy_global_1d_position], [self.dummy_local_1d_position], [self.dummy_filter_label_id]
        ...
        for segment_id, segment in enumerate(document):
            ...
            segment_text.append(segment_texts)

            filter_label = segment['filter_label']
            ...
            if isinstance(self.tokenizer, LayoutLMTokenizerFast):
                ...
                seg_input_ids = [b for b in tokenized_results['input_ids'][0] if b != space_id]
                seg_offset_mappings = [a for a, b in
                                    zip(tokenized_results['offset_mapping'][0], tokenized_results['input_ids'][0]) if
                                    b != space_id]
        ...
        assert len(input_ids) == len(content_mask)
        ...
        content_mask = content_mask[:self.max_text_length]
        ...
        if return_tensor:
            ...
            result['position_2d'] = torch.tensor(bboxes)[:, :4]

            #result['position_2d'] = torch.tensor(bboxes)
            result['labels'] = torch.tensor(labels)
            result['content_mask'] = torch.tensor(content_mask)
            ...
            result['position_2d'] = bboxes # TODO: remove h w columns
            result['labels'] = labels
            result['content_mask'] = content_mask
            result['segment_idxs'] = segment_idxs + ([0] * pad_len)

        if isinstance(self.tokenizer, LayoutLMTokenizerFast):
            result['position_ids'] = result.pop('position_1d') 
            result['bbox'] = result.pop('position_2d')
        if return_ref:
        ...
        result['segment_text'] = str(segment_text)
    ...
    def process_image(self, sample):
        ...
        ipath = os.path.join(self.data_dir, sample["image"])
        img = pil_loader(ipath)
        patch = torch.Tensor(self.image_processor(img)['pixel_values'][0])
...
if __name__ == '__main__':
    ...
    p = FilterPointerProcessor(tokenizer=tokenizer)
