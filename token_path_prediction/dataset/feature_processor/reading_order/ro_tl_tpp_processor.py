...
class CodeDocForTPPReadingOrderTLProcessor(CodeDocBaseProcessor):
    ...
    def process_text(self, sample, return_ref=False, return_tensor=True):
        ...
        (input_ids,
         ...
         global_1d_positions,
         local_1d_positions,
         filter_label_ids) = [self.tokenizer.cls_token_id], [1], [1], [self.dummy_bbox], [
        ...
        for segment_id, segment in enumerate(document):
            ...
            for i, (seg_token, seg_input_id, seg_offset_mapping) in enumerate(zip(seg_tokens, seg_input_ids, seg_offset_mappings)):
                ...
                if filter_label == 'D':
                    filter_label_ids.append(self.label2id['D'])
                    content_mask.append(0)
                elif filter_label == 'I' and len(seg_tokens)-1==i:
                    filter_label_ids.append(self.label2id['I'])
                    content_mask.append(1)
                else:
                    filter_label_ids.append(self.label2id['K'])
                    content_mask.append(1)
        ...
        assert len(input_ids) == len(filter_label_ids)
        ...
        filter_label_ids = filter_label_ids[:self.max_text_length]
        ...
        if return_tensor:
            ...
            result['filter_label_ids'] = torch.tensor(filter_label_ids)
            ...
            result['filter_label_ids'] = filter_label_ids
