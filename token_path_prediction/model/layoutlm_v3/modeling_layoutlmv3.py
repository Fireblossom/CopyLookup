...
from token_path_prediction.model.globalpointer import GlobalPointer
from token_path_prediction.model.multilabel_categorical_ce import globalpointer_loss
...
class LayoutLMv3Embeddings(nn.Module):
    ...
    def forward(
        ...
        if position_ids is None and self.config.enable_position_1d:
        ...
        if self.config.enable_position_1d:
            assert not position_ids is None
...
class LayoutLMv3ForTokenClassification(LayoutLMv3PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                active_attention_mask = attention_mask[:, :seq_length]
                active_loss = active_attention_mask.contiguous().view(-1) == 1
...
class LayoutLMv3ForTokenPathPrediction(LayoutLMv3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        # Added: 这两个是组内实用模型用来涨点的setting，LayoutLMv3这里我们暂时先不采用，之后再说
    ...
    def forward(self,
        ...
        if self.use_last_layer_position_embedding_residual:
            spatial_position_embeddings = self.layoutlmv3.embeddings._calc_spatial_position_embeddings(position_2d)
            # y_rel_dist_mask_mat = self.layoutlmv3.encoder._calculate_2d_y_rel_dist_mask_mat(position_2d)

            # 为v3文字编码部分的token加残差，要求position_embeddings的shape和sequence_output对齐
            img_len = int(sequence_output.shape[1] - spatial_position_embeddings.shape[1])
            if img_len > 0:
                spatial_position_embeddings = F.pad(spatial_position_embeddings, (0, 0, 0, img_len, 0, 0), 'constant', 0)

            sequence_output = sequence_output + spatial_position_embeddings
            if self.config.enable_position_1d:
                assert not position_1d is None
                position_embeddings_1d = self.layoutlmv3.embeddings.position_embeddings(position_1d)
                if img_len > 0:
                    position_embeddings_1d = F.pad(position_embeddings_1d, (0, 0, 0, img_len, 0, 0), 'constant', 0)
                sequence_output = sequence_output + position_embeddings_1d
...
class LayoutLMv3ForReadingOrder(LayoutLMv3ForTokenPathPrediction):
...
@dataclass
class LayoutLMv3ForFilterandReadingOrderOutput(ModelOutput):
    loss_filter: Optional[torch.FloatTensor] = None
    logits_filter: torch.FloatTensor = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None
    loss_pointer: Optional[torch.FloatTensor] = None
    logits_pointer: torch.FloatTensor = None
    

class LayoutLMv3ForFilterandReadingOrder(LayoutLMv3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.layoutlmv3 = LayoutLMv3Model(config)
        self.num_filter_labels = config.num_filter_labels
        classifier_dropout = (
            config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
        )
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        ...
        # classifier for filter
        self.loss_fct = CrossEntropyLoss()
        if config.num_filter_labels < 10:
            self.classifier = nn.Linear(config.hidden_size, config.num_filter_labels)
        else:
            self.classifier = LayoutLMv3ClassificationHead(config, pool_feature=False)
        # Initialize weights and apply final processing
        #self.post_init()
    ...
    def forward(
            ...
            images=None,
            **kwargs
        ) -> LayoutLMv3ForFilterandReadingOrderOutput:
        '''
        content_mask：在qa任务中，解码结果mask query部分
        '''
        bbox = position_2d
        ...
        sequence_output = outputs[0]
        logits_filter = self.classifier(self.dropout(sequence_output))

        loss_filter = None
        if filter_label_ids is not None:
            # Only keep active parts of the loss
            if attention_mask is not None:
                active_loss = attention_mask.view(-1) == 1
                active_logits = logits_filter.view(-1, self.num_filter_labels)
                active_labels = torch.where(
                    active_loss, filter_label_ids.view(-1), torch.tensor(self.loss_fct.ignore_index).type_as(filter_label_ids)
                )
                loss_filter = self.loss_fct(active_logits, active_labels)
            else:
                loss_filter = self.loss_fct(logits_filter.view(-1, self.num_filter_labels), filter_label_ids.view(-1))
        ...
        if self.training:
            ...
            predictions = torch.argmax(logits_filter, dim=-1)
            content_mask = 0
        logits_pointer = None
        loss_pointer = None
        if self.use_multi_dropout:
            if grid_labels is not None:
                ...
                for i, dropout in enumerate(self.dropouts):
                    sequence_output = dropout(sequence_output)
                    if i == 0:
                        logits_pointer, _ = self.global_pointer(sequence_output, attention_mask=content_mask)
                        loss_pointer = self.criterion(grid_labels, logits_pointer)
                    else:
                        logits_pointer_i, _ = self.global_pointer(sequence_output, attention_mask=content_mask)
                        logits_pointer = logits_pointer + logits_pointer_i
                        loss_pointer_i = self.criterion(grid_labels, logits_pointer_i)
                        loss_pointer = loss_pointer + loss_pointer_i

                logits_pointer = logits_pointer / len(self.dropouts)
                loss_pointer = loss_pointer / len(self.dropouts)
            else:
                # pred
                loss_pointer = None
                logits_pointer, _ = self.global_pointer(sequence_output, attention_mask=content_mask)
        else:
            logits_pointer, mask = self.global_pointer(sequence_output, attention_mask=content_mask)
            if grid_labels is not None:
                """
                    y_true:(batch_size, ent_type_size, seq_len, seq_len)
                    y_pred:(batch_size, ent_type_size, seq_len, seq_len)
        ...
        return LayoutLMv3ForFilterandReadingOrderOutput(
            loss_filter=loss_filter,
            logits_filter=logits_filter,
            loss_pointer=loss_pointer,
            logits_pointer=logits_pointer
        )
